This app was created as a part of excersise where I had to deliver web app in four hours. App was created in React. Data is fetched from cloud firestore.

<h1>Task description:</h1>

The goal of the exercise is to develop a small web application (front + back end + database) respecting the requirements defined below. To do so, you are free to choose the technologies you want and you feel more comfortable with. You have 4 hours to deliver your code and show us what you have done.

The specifications are voluntary light in order to let you express your creativity and imagination. We are also aware that the time allowed to complete the exercise is very limited. What is important for us is to see what you can produce, create and the quality of your code (structure, efficiency, performance, readability, cleanness...). In the context of this exercise, the design of the webpages is not important. 

<h2>Requirements</h2>

The application is a client ranking system that allows to enter new clients in the system and rank them according to an algorithm defined below.

The front end should consist of minimum 2 screens (html pages):
A page to add a new client with a form to input its data 
A summary page to list the ranking of the clients

The form to input the client’s data must contain the following fields:
<ul>
<li>Last name (must be filled in) </li>
<li>First name </li>
<li>Sex (must be filled in)  </li>
<li>Age (must be filled in) </li>
<li>Country of residence (dropdown list with some default values, must be filled in)  </li>
<li>Monthly salary (must be filled in) </li>
<li>Sources of income (dropdown list with some default values, must be filled in, displayed only if “Monthly salary” is above 10 000 EUR) </li>
<li>Contact data </li>
<ul>
<li>Email (required and unique) </li>
<li>Phone (optional) </li>
</ul>
<li>The email identifies uniquely a client. </li>
</ul>

The summary page will list the clients with their ranking in descending order. 
All the data relative to the client (including the rating) are saved in the DB.

<h3>Ranking computation </h3>
The ranking is an integer number between 0 and 100 and is computed as a weighted sum of 3 criteria:
<ul>
<li>Criterion 1: sex/age of client, weight: 10%
<li>Criterion 2: location, weight: 40%
<li>Criterion 3: salary, weight: 50%
</ul>

Each criterion has a result value between 0 and 100.

<h4>Criterion 1: </h4>
A man of 35 years old will get a result of 100 and a result of 0 at 7 years old and 77 years old. Between 35 years old and 7 years old, the result decrease linearly from 100 to 0. Idem between 35 years old and 77 years old.  The same rule applied for the woman taking into account that the 40 years old woman will get a result of 100. The lower (7) and upper (77) limit age are the same for both sex.

<h4>Criterion 2:</h4>
The client not living in Poland will be granted a result of 0.  Otherwise the result is 100.

<h4>Criterion 3:</h4
A man with a monthly salary > 10.000 EUR will get 100, between 5.000 EUR and 10.000 EUR will get 50 and below 5.000 EUR will get 0. For a woman we should considered the same ranges but with a 20% increased monthly salary.

If the client is blacklisted (lookup with a DB table that contains the blacklisted client names) or its age is &lt; 7 or &gt; 77, it's overall rating is 0. 

The clients with a rating &lt; 50 are not listed in the summary page. If 2 or more clients have the same ranking, the salary will determine the order in which the clients will appear on the summary page.

More, if you have some time!
<ol>
<li>For the field: “Sources of income”: to allow multiple choices. To adapt in DB with this new constraint.</li>
<li>By clicking on the name of the client in the summary, you can access and modify its data, the rating is then recomputed based on the new data of the client and the summary is automatically refreshed according to the new rating.
</ol>
